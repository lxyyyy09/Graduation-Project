...
object ReconditionedCliArgsIndexAccess : Macros {
    override val macroName: String = "reconditioned_cliargs_index_access"

    override fun toRust(): String {
        return """
            macro_rules! reconditioned_cliargs_index_access{
                (${"$"}a:expr,${"$"}b:expr) => {{
                    let arrLength = ${"$"}a.len();
                    let index = ${"$"}b;
                    if (index < arrLength) { index } else { 0 }
                }};
            }
        """.trimIndent()
    }
