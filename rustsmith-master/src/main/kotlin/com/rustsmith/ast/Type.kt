...
//            var keyLifetimePara:String="${type.keyType.lifetimeParameters().toSet().joinToString ( "," ){"'a$it"}}"
//            var valueLifetimePara:String="${type.valueType.lifetimeParameters().toSet().joinToString ( "," ){"'a$it"}}"
//            return "HashMap<<$keyLifetimePara>${type.keyType.toRust()},<$valueLifetimePara>${type.valueType.toRust()}>"
//            return "reallyconfuse"
//        }
...
data class TraitType(
    ...
    override fun clone(): Type {
        return TraitType(traitName,traitFunctions,traitMap,symbolTable)
    }
...
@GenNode
data class RcType(val internalType: Type) : RecursiveType {
    override val argumentsToOwnershipMap: MutableList<Pair<Type, OwnershipState>> = mutableListOf()

    override fun clone(): Type = RcType(internalType.clone())
    override fun memberTypes(): List<Type> = listOf(this) + internalType.memberTypes()
    override fun lifetimeParameters(): List<UInt> = internalType.lifetimeParameters()

    override fun toRust(): String {
        return "Rc<${internalType.toRust()}>"
    }
}
...
fun Type.getOwnership(): OwnershipModel {
    return when (this) {
        ...
        is HashMapType -> OwnershipModel.MOVE
        is BoxType -> OwnershipModel.MOVE
        is RcType -> OwnershipModel.MOVE
