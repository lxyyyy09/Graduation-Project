...
//                .filter { tupleType.argumentsToOwnershipMap[it].second != OwnershipState.INVALID }
    ...
    override fun generateNewRcExpression(type: Type, ctx: Context): NewRcExpression {
        if (type is RcType) {
            ...
            return NewRcExpression(internalExpression, symbolTable)
        }
        throw IllegalArgumentException("Not a Rc type")
    }

    override fun generateNewHashMapExpression(
        type: Type,
        ctx: Context
    ...
    override fun generateStringLengthExpression(type: Type, ctx: Context): StringLengthExpression {
        val stringExpression = generateExpression(
            generateStringType(ctx.incrementCount(StringLengthExpression::class)),
            ctx.incrementCount(StringLengthExpression::class)
        )
        return StringLengthExpression(
            stringExpression, symbolTable
        )
    }
...
//            ctx.incrementCount(StringPushStrExpression::class)
//        )
    }
...
//    }
    ...
    override fun generateRcType(ctx: Context): RcType {
        ...
        if (randomRcType == null || selectionManager.choiceGenerateNewTupleWeightings(ctx).randomByWeights()) {
            val internalType = generateType(ctx.incrementCount(RcType::class))
            symbolTable.globalSymbolTable.addRcType(internalType)
            return RcType(internalType)
        }
